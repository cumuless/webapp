...
const getIcon = (page: string) => {
  switch (page) {
    ...
    case 'email':
      return <EnvelopeClosedIcon width='20px' height='20px' />;
    default:
      return null;
  }
};
...
const NavBar = () => {
  const [availablePages, name, setAppearance] = useStore(
    (s) => [s.availablePages, s.name, s.setAppearance],
    shallow
  );
  ...
  return (
    <Flex style={{ background: 'var(--gray-3)' }}>
      <Flex
        width='54px'
        direction='column'
        style={{ margin: '16px 6px 10px 6px', justifyContent: 'space-between' }}
        align={'center'}
      >
        <Flex direction='column' gap='3'>
          ...
          {availablePages.map((page, index) => (
            <Link href={`/${page.page}`} key={index}>
              <Flex>
                <Tooltip
                  content={snakeToTitleCase(page.page)}
                  delayDuration={0}
                  side='right'
                >
                  <IconButton
                    key={page.page}
                    variant={
                      pathname.slice(1, page.page.length + 1) === page.page
                        ? 'soft'
                        : 'outline'
                    }
                    size='4'
              ...
              <DropdownMenu.Content>
                <DropdownMenu.Sub>
                  ...
                  <DropdownMenu.SubContent>
                    <DropdownMenu.Item onClick={() => setAppearance('light')}>
                      Light
                    </DropdownMenu.Item>
                    <DropdownMenu.Item onClick={() => setAppearance('dark')}>
                      Dark
                    </DropdownMenu.Item>
                  </DropdownMenu.SubContent>
                </DropdownMenu.Sub>
