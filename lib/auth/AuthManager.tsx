...
import { getErrorMessage, goto, log, logError, logSuccess, logUnkownIssue, logWarning, tokenSafeString } from "@utils";
import { confirmSignIn, fetchUserAttributes, getCurrentUser, resendSignUpCode, signIn, signOut } from "aws-amplify/auth";
...
export async function isAuthenticated() {
  ...
  if(tokens?.accessToken?.toString()) {
    logSuccess(`User is Authenticated with token ${tokenSafeString(tokens?.accessToken?.toString())}`)
    return true;
  }
...
export async function logIn(creds: LoginData): Promise<string | boolean> {
  try {
    ...
    else if(signInStep === 'CONFIRM_SIGN_IN_WITH_NEW_PASSWORD_REQUIRED') return 'PASSWORD_RESET_REQUIRED'
    else {
...
export async function setUserPassword(creds: LoginData) {
  try {
    let {nextStep: {signInStep}, isSignedIn} = await confirmSignIn({challengeResponse: creds.password ?? '', options: {userAttributes: {name: creds.name ?? ''}}});
    logSuccess(`Logged in with status: ${isSignedIn} and next step ${signInStep}`)
    ...
    switch (e.name) {
      ...
      case 'InvalidPasswordException':
        showErrorPopup("Password must be >7 characters with >1 number, special character, uppercase, and lowercase letter")
        break;
      default:
